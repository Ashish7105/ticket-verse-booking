...
const Navbar: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState([]); // Example: fetch or hardcode results
  ...
  const handleSearch = (query: string) => {
    ...
    // Simulate search results (you can integrate real search logic here)
    if (query) {
      setSearchResults([
        { id: 1, title: "Movie 1", genre: "Action", poster: "/path/to/poster1.jpg" },
        { id: 2, title: "Movie 2", genre: "Drama", poster: "/path/to/poster2.jpg" },
      ]);
    } else {
      setSearchResults([]);
    }
  ...
  const handleSearchResultClick = (id: number) => {
    ...
    setSearchVisible(false); // Hide search results after click
  };
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className={`hidden md:flex items-center relative flex-1 max-w-xl mx-4 ${searchVisible ? 'absolute top-16 left-0 right-0 p-4 flex bg-white shadow-md z-50' : ''}`}>
            ...
            {searchQuery && searchResults.length > 0 && (
              <div className="absolute mt-2 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-[300px] overflow-y-auto z-50">
                {searchResults.map((movie) => (
                  ...
                  >
                    <img src={movie.poster} alt={movie.title} className="w-8 h-12 object-cover rounded mr-3" />
                    <div>
